...
parser.add_argument("--out-dir", type=str, default=config['initial_checkpoint'][:-15],
...
def eval(net, dataset, save_dir=None):
    ...
    dice_dir = os.path.join(save_dir, 'DICE')
    if not os.path.exists(dice_dir):
        os.makedirs(dice_dir)
    diceOutputfile = open(os.path.join(dice_dir, 'Dice_mAP.txt'), 'w')
    diceOutputfile.write('mAP: ' + str(np.mean(aps, 0)) + '\n')
    diceOutputfile.write('mean dice:%.4f(%.4f)\n' % (np.mean(dices), np.std(dices)))
    diceOutputfile.write('mean dice (exclude fn):%.4f(%.4f)\n' % (np.mean(dices[dices != 0]), np.std(dices[dices != 0])))
    ...
    noduleCADEvaluation('evaluationScript/annotations/new_coord_annotations.csv',
                        'evaluationScript/annotations/new_excluded_coord_annotations.csv',
                        dataset.set_name, rpn_submission_path, os.path.join(eval_dir, 'rpn'))
...
if __name__ == '__main__':
    ...
    # os.makedirs('results/noduleNet20220926-201433/res200/FROC/ensemble_new')
    # noduleCADEvaluation('evaluationScript/annotations/new_coord_annotations.csv',
    #                     'evaluationScript/annotations/new_excluded_coord_annotations.csv',
    #                     'split/val0.csv', 'results/noduleNet20220926-201433/res200/FROC/submission_ensemble.csv',
    #                     'results/noduleNet20220926-201433/res200/FROC/ensemble_new')
    # os.makedirs('results/noduleNet/FROC/ensemble_new')
    # noduleCADEvaluation('evaluationScript/annotations/new_coord_annotations.csv',
    #                     'evaluationScript/annotations/new_excluded_coord_annotations.csv',
    #                     'split/seriesuids.csv', 'results/noduleNet/submission_ensemble.csv',
    #                     'results/noduleNet/FROC/ensemble_new')
