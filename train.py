...
from net.Tsc_nodule_net import TSC-Nodule
import time
...
def main():
    ...
    ####################################################
    print("dataloader_start:", time.time())
    train_dataset_list = []
    val_dataset_list = []
    for i in range(len(train_set_list)):
        set_name = train_set_list[i]  # 'split/train3.csv' (710)
        label_type = label_types[i]  # 'mask'
    ...
    ##################################################
    print("dataloader_end:", time.time())
    ...
    # Initilize network
    ##############################################
    print("init_model_start:", time.time())
    net = getattr(this_module, net)(net_config)
    print("model_creation_end:", time.time())
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("device_assignment_end:", time.time())

    net.to(device)  # 将模型移到设备上
    print("model_to_device_end:", time.time())
    print("model device:", next(net.parameters()).device)
    print("init_model_end:", time.time())
...
def train(net, train_loader, optimizer, epoch, writer):
    ...
    for j, (input, truth_box, truth_label, truth_mask, masks) in tqdm(enumerate(train_loader), total=len(train_loader),
                                                                      desc='Train %d' % epoch):
        input = Variable(input).cuda()
        truth_box = np.array(truth_box, dtype=object)
        truth_label = np.array(truth_label, dtype=object)
        truth_mask = np.array(truth_mask, dtype=object)
    ...
    # 遍历每个内部列表
    for inner_list in rpn_stats:
        new_inner_list = []

        # 遍历内部列表中的每个元素
        for item in inner_list:
            if isinstance(item, torch.Tensor):  # 如果是张量，将其转移到CPU并转换为NumPy数组
                new_inner_list.append(item.cpu().numpy())
            else:  # 对于其他类型（int或float），直接添加
                new_inner_list.append(item)

        # 将处理后的内部列表添加到新的统计数据列表中
        new_rpn_stats.append(new_inner_list)

    # 最后将列表转换为NumPy数组
    rpn_stats = np.array(new_rpn_stats, dtype=np.float32)
...
def validate(net, val_loader, epoch, writer):
    ...
    s = time.time()
    for j, (input, truth_box, truth_label, truth_mask, masks) in tqdm(enumerate(val_loader), total=len(val_loader),
                                                                      desc='Val %d' % epoch):
        with torch.no_grad():
            input = Variable(input).cuda()
