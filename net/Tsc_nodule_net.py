...
from net import tbc_cgnl
from net.ScConv_3D import ScConv_3D as SCR_3D
from net.squeeze_and_excitation_3D import ProjectExciteLayer as PE
...
class SCR_ResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, stride=1):
        super(SCR_ResBlock3d, self).__init__()
        self.conv1 = nn.Conv3d(n_in, n_out, kernel_size=3, stride=stride, padding=1)
        self.bn1 = nn.BatchNorm3d(n_out, momentum=bn_momentum)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(n_out, n_out, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        out += residual
        # print("residual.out: ", out.shape)
        out = self.relu(out)
        # print("out: ", out.shape)
        return out


class TSCR_Res_CGNL(nn.Module):
    def __init__(self, inplanes, planes, use_scale=False, groups=4):
        super(TSCR_Res_CGNL, self).__init__()
        self.use_scale = use_scale
        self.groups = groups
        self.tbc_cgnl = tbc_cgnl.SpatialCGNL_TBC(inplanes, planes, use_scale, self.groups)
        self.scr_res = SCR_ResBlock3d(inplanes,inplanes)
        # self.pe = PE(inplanes)
...
#         self.pkinet = PKINet(arch='NoduleNet', in_channels=in_channels, drop_path_rate=0.1)
#
...
#
#     def forward(self, x):
#         outs, outs_pool = self.pkinet(x)
#         out, out1, out2, out3, out4 = outs
#         out_pool, out1_pool, out2_pool, out3_pool, out4_pool = outs_pool
#
...
class FeatureNet(nn.Module):
    def __init__(self, config, in_channels, out_channels):
        ...
        self.forw2 = nn.Sequential(
            ResBlock3d(32, 64),
            # HybridBlock(64, 64, weight_type=None),
            # ResBlock3d(64, 64)
            # SCR_ResBlock3d(64,64)
            TSCR_Res_CGNL(64,128)
        )
...
class MaskHead(nn.Module):
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            up1 = f_4[b, :, int(z_start / 4):int(z_end / 4), int(y_start / 4):int(y_end / 4),
                  int(x_start / 4):int(x_end / 4)].unsqueeze(0)
            up2 = self.up2(up1)
            up2 = self.back2(torch.cat(
                (up2, f_2[b, :, int(z_start / 2):int(z_end / 2), int(y_start / 2):int(y_end / 2),
...
class TSC-Nodule(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(NoduleNet_TSCR, self).__init__()
    ...
    def forward(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        ...
        self.rpn_window = make_rpn_windows(fs, self.cfg)  # 在特征图上以不同中心点生成不同大小的锚框，中心点步长和锚框大小信息在cfg中
        self.rpn_proposals = []
        if self.use_rcnn or self.mode in ['eval', 'test']:
            ...
            # rpn_nms:将rpn_deltas_flat（回归项）运用到rpn_window（锚框），生成输入CT上的候选框，然后进行非极大值抑制
            # self.rpn_proposals: (x, 8) (其中8为[b, logits, x, y, z, d, h, w]) 由batch_size、rpn_logits_flat、rpn_deltas_flat、rpn_window一起生成的候选信息
        ...
        if self.mode in ['train', 'valid']:
            ...
            # make_rpn_target: 为训练RPN生成目标，包括每个锚框的标签（是否包含物体）和回归目标（如何调整锚框以更好地匹配真实边界框）
            # 取出每一个rpn_deltas_flat（它的维度和生成锚框初始维度相同）对应的锚框在所有truth_box中与该锚框IOU最大的那一个
            # 该rpn_deltas_flat学习的就是这个truth_box的x,y,z,d,h,w
            # 而rpn_logits_flat学习的是它对应的锚框被判断为前景还是背景
            if self.use_rcnn:
                # self.rpn_proposals = torch.zeros((0, 8)).cuda()
        ...
        if self.use_rcnn:
            if len(self.rpn_proposals) > 0:
                ...
                if self.mode in ['eval']:  #### I tab these to index
                    # Ensemble
                ...
                # Ensure ensemble_proposals and fpr_res have the expected dimensions
                if self.ensemble_proposals.dim() == 1:
                    self.ensemble_proposals = self.ensemble_proposals.unsqueeze(1)
                    if fpr_res.dim() == 1:
                        fpr_res = fpr_res.unsqueeze(1)
    ...
    def forward_mask(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        ...
        for b in range(len(truth_boxes)):
            self.crop_boxes.append(
                np.column_stack((np.zeros((len(truth_boxes[b]) + b, 1)), truth_boxes[b], truth_labels[b])))
        self.crop_boxes = np.concatenate(self.crop_boxes, 0)
        self.crop_boxes[:, 1:-1] = center_box_to_coord_box(self.crop_boxes[:, 1:-1])
        self.crop_boxes = self.crop_boxes.astype(np.int32)
        self.crop_boxes[:, 1:-1] = ext2factor(self.crop_boxes[:, 1:-1], 4)
...
if __name__ == '__main__':
    net = TSC-Nodule(config)
    print(net)
    total = sum([para.nelement() for para in net.parameters()])
    print("Numberof parameter: % .2fM" % (total / 1024 ** 2))

    input = torch.rand([4, 1, 128, 128, 128])
    input = Variable(input)
    output = net.feature_net(input)
    print(output)
