...
from net.sc_conv import SCConv, SCBottleneck
from net.cbam3d import ChannelGate, SpatialGate
from net.TiedBlockConv3d import TiedBlockConv3d as tbc


class SpatialCGNL_TBC(nn.Module):
    ...
    def __init__(self, inplanes, planes, use_scale=False, groups=None):
        ...
        super(SpatialCGNL_TBC, self).__init__()
        # conv theta
        # self.t = nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, bias=False)
        self.t = tbc(inplanes, planes, kernel_size=1, stride=1, bias=False)
        # conv phi
        # self.p = nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, bias=False)
        self.p = tbc(inplanes, planes, kernel_size=1, stride=1, bias=False)
        # conv g
        # self.g = nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, bias=False)
        self.g = tbc(inplanes, planes, kernel_size=1, stride=1, bias=False)
        # self calibrated conv
        # self.sc = SCBottleneck(planes,planes)
        # conv z
        # self.z = nn.Conv3d(planes, inplanes, kernel_size=1, stride=1,
        #                                           groups=self.groups, bias=False)
        self.z = tbc(planes, inplanes, kernel_size=1, stride=1,
                           groups=self.groups, bias=False)

        self.gn = nn.GroupNorm(num_groups=self.groups, num_channels=inplanes)

        # self.channel_gate = ChannelGate(inplanes)
        # self.spatial_gate = SpatialGate()

        if self.use_scale:
            cprint("=> WARN: SpatialCGNL_TBC block uses 'SCALE'", \
                   'yellow')
        if self.groups:
            cprint("=> WARN: SpatialCGNL_TBC block uses '{}' groups".format(self.groups), \
                   'yellow')
    ...
    def forward(self, x):
        ...
        if x.size()[2:] == residual.size()[2:]:
            ...
            x = self.gn(F.interpolate(x, size=residual.size()[2:], mode='trilinear', align_corners=False)) + residual

        return x



# class SpatialCGNL_SC(nn.Module):
#     """Spatial CGNL block with dot production kernel for image classfication.
#     """
#     def __init__(self, inplanes, planes, use_scale=False, groups=None):
#         self.use_scale = use_scale
#         self.groups = groups
#
#         super(SpatialCGNL_SC, self).__init__()
#         # conv theta
#         self.t = nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, bias=False)
#         # conv phi
#         self.p = nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, bias=False)
...
if __name__ == '__main__':
    ...
    input = torch.randn(size=(2, 32, 64, 64, 64))

    model = SpatialCGNL_TBC(inplanes=32, planes=64,  use_scale=False, groups=1)
    # model = SpatialCGNL_SCR(inplanes=32, planes=64, use_scale=False, groups=4)
    print(model)
