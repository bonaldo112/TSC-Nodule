...
def box_transform(windows, targets, weight):
    ...
    dx = wx * (tx - bx) / bw
    ###############################################################
    # dd = wd * np.log(td.astype(np.float) / bd.astype(np.float))
    # dh = wh * np.log(th.astype(np.float) / bh.astype(np.float))
    # dw = ww * np.log(tw.astype(np.float) / bw.astype(np.float))
    dd = wd * np.log(td.astype(float) / bd.astype(float))
    dh = wh * np.log(th.astype(float) / bh.astype(float))
    dw = ww * np.log(tw.astype(float) / bw.astype(float))
    ################################################################
