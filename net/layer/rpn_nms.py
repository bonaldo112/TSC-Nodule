...
def rpn_nms(cfg, mode, inputs, window, logits_flat, deltas_flat):
    ...
    for b in range(batch_size):
        ...
        if len(index) > 0:
            ...
            d = ds[index]
            w = window[index]
            box = rpn_decode(w, d, cfg['box_reg_weight'])  # 将回归项应用到锚框上，生成新的预测边界框
            box = clip_boxes(box, inputs.shape[2:])  # 由于新的预测边界框大小不确定，故需要裁剪到图像的大小内

            output = np.concatenate((p, box),1)

            output = torch.from_numpy(output)
            output, keep = torch_nms(output, nms_overlap_threshold)  # 删除所有与已保留边界框重叠度超过nms_overlap_threshold的边界框,keep是索引
