...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    ...
    # sampled_target = Variable(torch.from_numpy(sampled_target.astype(np.float))).float().cuda()
    # sampled_label = Variable(torch.from_numpy(sampled_label.astype(np.float))).long().cuda()
    sampled_target = Variable(torch.from_numpy(sampled_target.astype(float))).float().cuda()
    sampled_label = Variable(torch.from_numpy(sampled_label.astype(float))).long().cuda()
