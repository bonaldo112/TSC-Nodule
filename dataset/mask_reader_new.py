    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                sample, target, masks = self.crop(imgs, [], bboxes, do_scale=False, isRand=True)
        ...
        if self.mode in ['eval']:
            ...
            #添加split_comber
            nz, nh, nw = image.shape[1:]
            pz = int(np.ceil(float(nz) / self.stride)) * self.stride
            ph = int(np.ceil(float(nh) / self.stride)) * self.stride
            pw = int(np.ceil(float(nw) / self.stride)) * self.stride
            image = np.pad(image, [[0, 0], [0, pz - nz], [0, ph - nh], [0, pw - nw]], 'constant',
                          constant_values=self.pad_value)

            xx, yy, zz = np.meshgrid(np.linspace(-0.5, 0.5, image.shape[1] // self.stride),
                                     np.linspace(-0.5, 0.5, image.shape[2] // self.stride),
                                     np.linspace(-0.5, 0.5, image.shape[3] // self.stride), indexing='ij')
            coord = np.concatenate([xx[np.newaxis, ...], yy[np.newaxis, ...], zz[np.newaxis, :]], 0).astype('float32')
            image, nzhw = self.split_comber.split(image)
...
if __name__ == '__main__':
    ...
    data_dir = 'H:\\datasets\\LUNA16\\' + 'preprocessed_test/3'
    set_name = 'split/val0.csv'
    dataset = mask_reader_new(data_dir, set_name, config, mode='eval')
    print(len(dataset))
    inputs = dataset[0]
    img, truth_bboxes, truth_labels, truth_masks, masks,_ = inputs
    print(truth_bboxes)
    print(truth_labels)
    print(truth_masks.shape)
    print(masks.shape)
