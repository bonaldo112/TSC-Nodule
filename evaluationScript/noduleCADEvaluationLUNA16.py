...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    #         return None
    #
    #     # Calculate the absolute differences between elements and the target value
    ...
    #         return None
    #
    #     diff = valid_fps - value
    #     min_diff = np.nanmin(diff)
    #     if np.isnan(min_diff):
    #         return None
    #
    #     idx = np.where(diff == min_diff)[0][-1]
    #     return idx

    thres = [0.125, 0.25, 0.5, 1, 2, 4, 8]
    sen = []
    print('FROC at points: ', thres)
    nodOutputfile.write('FROC at points: 0.125, 0.25, 0.5, 1, 2, 4, 8\n')
    for th in thres:
        print('fps: ', th, ', sensitivity: ', sens[find_nearest(fps, th)])
        nodOutputfile.write('fps: %.3f, sensitivity: %.8f\n' % (th, sens[find_nearest(fps, th)]))
