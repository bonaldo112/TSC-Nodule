...
def find_mhd_file(subset_path, patient_id):
    for subject_no in range(0, 10):
        src_dir = subset_path + "subset" + str(subject_no) + "/"
...
def preprocess(params):
    ...
    file_name = find_mhd_file(img_dir, pid)
    # img, origin, spacing = load_itk_image(os.path.join(img_dir, '%s.mhd' % (pid)))
    img, origin, spacing = load_itk_image(file_name)
    ...
    # 存储origin,spacing,ebox用于还原检测框中心在原始数据中的世界坐标系位置
    # coord=(center+ebox)*spacing+origin
...
def main():
    ...
    # extra_nod_dir = os.path.join(config['mask_save_dir'], str(0))
    ...
    params_lists = []
    # for pid in os.listdir(nod_mask_dir) + os.listdir(extra_nod_dir):
