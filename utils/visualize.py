...
def plot_compare_figure(image, gt, pred, params, save_dir, show_all_legend, fmt=['png']):
    ...
    if show_all_legend:
        first_legend = plt.legend(handles=patches1, bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0.,
...
patches2 = [patches.Patch(color=cmaplist2[i], label="{}".format(legend_names[i]), fill=False) for i in
            range(len(cmaplist2))]
